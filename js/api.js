...
## 🚨 关键说明：专业分析要求
1. **真实数据分析**：基于用户提供的店铺运营数据进行专业分析
2. **图表数据**：系统将生成的演示数据 → 严禁在文字分析中引用

## 重要提醒：基于实际运营数据进行专业商业分析
请仔细观察并提取店铺运营数据中的所有关键信息，包括但不限于：数值、百分比、趋势、排名、评分等。
⚠️ 所有文字分析内容（SWOT、建议、洞察）必须基于真实的运营数据，严禁引用任何图表中的演示数据。
...
#### 2. 销售数据（如果数据中可见）
- 日订单量：XXX单（必须是实际数据中的确切数字）
- 月订单量：XXX单
- 营业额：XXX元
- 客单价：XX元
- 销售趋势：上升/下降/稳定（基于实际趋势数据）

#### 3. 评价数据（如果数据中可见）
- 店铺评分：X.X分（必须是实际数据中的确切分数）
- 评价总数：XXX条
- 好评率：XX%
- 差评关键词：[从实际数据中提取的具体差评内容]
...
### 分析要求：
- 在SWOT分析中，必须基于具体的运营数据进行专业分析
- 在优化建议中，必须针对发现的具体问题提出解决方案
- 在趋势分析中，必须基于实际的业务趋势数据
...
**必须使用真实运营数据**：
- 所有数值必须从实际运营数据中提取
- 分析内容应呈现为专业的商业洞察，避免提及数据来源
- 如果某项数据不可获得，不得编造或使用演示数据
- 优势劣势必须基于实际的业务表现
...

{
  ...
  "analysis": {
    "strengths": [
      ...
      "必须引用实际数据中的确切数值，禁止使用4.3分、156单、35分钟、30元等演示数据",
      "每个优势点都必须有具体的业务数据支撑，呈现为直接的商业洞察"
    ],
    "weaknesses": [
      "基于发现的真实问题进行专业分析，例如：'下单转化率为X.X%，低于同行均值X.X%，存在明显的转化效率提升空间'",
      "严禁使用配送时间35分钟、客单价30元等演示数据，必须使用实际运营数据",
      "每个劣势点都必须基于实际业务数据，呈现为专业的问题识别"
    ],
    "opportunities": [
      "基于数据差距和改进空间进行专业分析，识别具体的增长机会",
      "严禁使用通用的机会描述，必须针对具体的业务数据情况",
      "每个机会点都要有真实数据支撑，如转化率差距、同行对比等"
    ],
    "threats": [
      "基于与同行对比的真实劣势数据进行专业分析",
      "严禁使用通用的威胁描述，必须基于具体的竞争风险点",
      "每个威胁点都要有数据依据，如竞争对手表现更好的具体指标"
    ],
    "recommendations": [
      {
        "category": "基于业务数据的具体问题",
        "suggestion": "针对发现的具体问题提出解决方案，如：'当前转化率为X.X%，建议优化商品展示页面提升用户体验'",
        "priority": "基于问题的严重程度确定优先级",
        "expectedImpact": "基于数据差距预估具体的改进效果，如：'预期转化率提升至X.X%'",
        "implementationSteps": ["针对具体问题的可执行步骤", "步骤具体可操作", "有明确的执行标准"]
      },
      {
        "category": "基于数据差距的改进机会",
        "suggestion": "针对与同行的具体差距提出改进建议",
        "priority": "基于数据影响程度确定",
        "expectedImpact": "量化预期改进效果",
        "implementationSteps": ["具体的执行步骤", "可衡量的执行标准", "明确的时间节点"]
      },
      {
        "category": "基于实际表现的优化方案",
        "suggestion": "针对具体运营状况提出优化建议",
        "priority": "基于实际数据确定",
        "expectedImpact": "基于现状预估改进空间",
    ...
    "insights": [
      "基于具体业务数据得出的深度洞察，如：'X.X%的转化率表明店铺在用户转化方面具有显著优势...'",
      "结合实际运营情况进行行业对比分析",
      "基于与同行的数据差距提出竞争策略洞察"
    ]
  },
...
### 数据来源明确区分：
1. **运营数据**：从用户提供的店铺运营数据中提取的真实信息（这是分析的唯一依据）
2. **图表数据**：系统生成的演示数据（仅用于图表展示，不得在文字分析中引用）

### 必须遵循的分析原则：
1. **仅使用真实数据**：所有文字分析内容（SWOT、建议、洞察）必须且仅能基于实际运营数据
2. **禁止引用图表数据**：不得在文字分析中引用任何图表中的演示数据
3. **专业表述方式**：分析内容应呈现为直接的商业洞察，如"转化率达到X.X%，表明..."
4. **对比分析**：将本店数据与同行数据进行专业对比
5. **问题针对性**：发现的问题基于实际业务表现
...
- ✅ "与同行X.X%的差距显示出竞争劣势，优先改进..."
- ✅ 如果某项数据不可获得，标注"该数据项暂无法获取"

## 专业分析要求
1. **数据驱动决策**：基于可获得的具体运营数据进行分析，对无法获取的数据项标注"暂无数据"
2. **精确数值引用**：在分析中必须引用实际的确切数值，不得使用估算或假设数据
3. **可操作性建议**：每条建议都要针对具体的业务问题，包含实施步骤和预期效果
4. **ROI导向**：重点关注最需要改进的关键业务环节
5. **竞争分析**：基于同行对比数据进行差异化策略分析
